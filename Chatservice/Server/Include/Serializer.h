...
public:
    ...
    template<typename T> T ExtractArray(size_t length)
    {
        typedef typename std::enable_if<std::is_pointer<T>::value || std::is_array<T>::value>::type Typecheck;
        typedef typename std::conditional<
            (std::is_pointer<typename std::remove_pointer<T>::type>::value), PtrptrArg, PtrArg
        >::type TArg;
        return ExtractArray<T>(length, TArg());
    }
...
private:
    ...
    template<typename U> U ExtractArray(size_t length, PtrArg)
    {   // U is a pointer type
        typedef typename std::enable_if<
            (std::is_pointer<U>::value || std::is_array<U>::value) &&
        ...
        typedef typename std::remove_pointer<U>::type T;
        if ((m_ptail - m_ppos) >= length * sizeof(T)) {
            T *ptemp = new T[length];
            std::memcpy(ptemp, m_ppos, length * sizeof(T));
            m_ppos += length * sizeof(T);
            return ptemp;
        }
        return nullptr;
    }
    template<typename U> U ExtractArray(size_t length, PtrptrArg)
    {   // U is a pointer to pointer
        typedef typename std::enable_if<
            (std::is_pointer<U>::value || std::is_array<U>::value) &&
            (std::is_pointer<typename std::remove_pointer<U>::type>::value ||
